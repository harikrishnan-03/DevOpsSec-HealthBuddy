...
def profile(request):
    ...
    return render(request,"Profile.html",{'filterUser':filterUser,'dob':filterUserDob,'gender':filterUserGender})
...
def addMedicine(request):
    if request.method=='POST':
        ...
        saveMedicine=CurrentMedicine(user=currentUser,medicine_name=medicineName,medicine_time=medicineTime)
        saveMedicine.save()
...
def updateMedicine(request,id):
    ...
    return render(request,"UpdateMedicine.html",{'id':medicineToUpdate,'medicine_time':medicine_time })
...
def addVaccine(request):
    if request.method=='POST':
        ...
        saveVaccine=VaccineList(user=currentUser,vaccine_name=vaccineName,vaccine_startdt=vaccineStart,vaccine_enddt=vaccineEnd)
        saveVaccine.save()
...
def updateVaccine(request,id):
    ...
    return render(request,"UpdateVaccines.html",{'id':vaccineToUpdate,'vaccine_startdt':vaccine_startdt,'vaccine_enddt':vaccine_enddt})
...
def insuranceList(request):
    ...
    print(currentUser)
    return render(request,"Insurance.html",{'insuranceDetails':insuranceDetails}) #Stored as key value pairs
...
def addInsurance(request):
    if request.method=='POST':
        ...
        saveInsurance=HealthInsurance(user=currentUser,insurance_policy_name=policyName,insurance_premium=premium,insurance_startdt=startDate, insurance_enddt=endDate)
        saveInsurance.save()
...
def updateInsurance(request,id):
    ...
    return render(request,"UpdateInsurance.html",{'id':insuranceToUpdate,'insurance_startdt':insurance_startdt,'insurance_enddt':insurance_enddt})
...
def listLabResult(request):
    ...
    print(labDetails)
    print(currentUser)
    return render(request,"LabResult.html",{'labDetails':labDetails}) #Stored as key value pairs
...
def addLabResult(request):
    if request.method=='POST':
        ...
        saveLabResult=LabTestResults(user=currentUser,lab_test_date=testDate,lab_sugar=sugar,lab_pressure=pressure,lab_Cholesterol=cholesterol)
        saveLabResult.save()
...
def updateLabResult(request,id):
    ...
    return render(request,"UpdateLabResult.html",{'id':labResultToUpdate,'lab_test_date':lab_test_date})
...
def listAllergies(request):
    ...
    print(currentUser)
    return render(request,"Allergies.html",{'allergyDetails':allergyDetails}) #Stored as key value pairs
...
def addAllergies(request):
    if request.method=='POST':
        ...
        saveAllergy=AllergicHistory(user=currentUser,gen_allergy_name=allergyName,gen_allergy_description=allergyDescription)
        saveAllergy.save()
...
def listAllergicMedicine(request):
    ...
    print(currentUser)
    return render(request,"AllergicMedicine.html",{'allergyMedicineDetails':allergyMedicineDetails}) #Stored as key value pairs
...
def addAllergicMedicine(request):
    if request.method=='POST':
        ...
        saveAllergicMedicine=AllergicMedicine(user=currentUser,med_allergy_name=medAllergyName,med_allergy_description=medAllergyDescription)
...
def updateAllergicMedicine(request,id):
    ...
    return render(request,"UpdateAllergicMedicine.html",{'id':allergicMedicineToUpdate })
...
def updateAllergicMedicineForm(request,id):
        if request.method=='POST':
            ...
            allergicMedicineToUpdate.med_allergy_description=request.POST['medAllergyDescription']
