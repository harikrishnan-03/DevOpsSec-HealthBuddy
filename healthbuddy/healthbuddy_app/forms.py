...
class CustomUserForm(UserCreationForm):
    ...
    mobileNumber = forms.IntegerField(
        widget=forms.NumberInput(attrs={"class": "form-control", "name": "mobNo"})
    )
    dob = forms.DateField(
        widget=forms.DateInput(
            attrs={"class": "form-control", "type": "date", "name": "dob"}
        )
    )
    height = forms.IntegerField(
        widget=forms.NumberInput(attrs={"class": "form-control", "name": "height"})
    )
    weight = forms.IntegerField(
        widget=forms.NumberInput(attrs={"class": "form-control", "name": "weight"})
    )
    bloodGroup = forms.ChoiceField(
        choices=BLOOD_GROUP,
        required=True,
        widget=forms.Select(attrs={"class": "form-control", "name": "bloodGroup"}),
    )
